...
O backend expoe a API local enquanto a persistencia do frontend continua em `localStorage` no navegador por enquanto.
...
Os dados em `data/` tem dois papeis diferentes:

- `python backend/scripts/sync_data.py` baixa/sincroniza os datasets SRD e atualiza `data/manifest.json`, que o backend usa em runtime.
- `python backend/scripts/build_5etools_data.py ./5etools-v2.28.0` gera `data/5etools/...` a partir de uma copia local do 5etools para o fluxo compacto do character builder.

Em uma configuracao nova, rode primeiro o sync para garantir que o manifest do backend exista e esteja atualizado. Depois rode o build do 5etools se voce tambem precisar dos dados compactados locais.

Os dados baixados/gerados ficam em:
...
Para sincronizar os datasets SRD usados pelo backend e atualizar o manifest:
...
```bash
python backend/scripts/build_5etools_data.py ./5etools-v2.28.0
```
