...
function renderEquipmentChoice(rule) {
  return builderRenderers.renderEquipmentChoice(rule, creationChoicesLocked());
}

function renderClassCreationChoice(rule) {
  return builderRenderers.renderClassCreationChoice(rule);
}

function renderAsiChoice(rule) {
  return builderRenderers.renderAsiChoice(rule);
}
...
function renderBackgroundForm() {
  ...
  return renderTypedBackgroundForm({
    ...
    titleCase,
    escapeHtml,
    renderBgSpellChoices: () => backgroundSpellChoiceRules().map(builderRenderers.renderBgSpellChoice).join(""),
  });
}
...
function renderLevelingForm() {
  ...
  return renderTypedLevelingForm({
    ...
    navButtonsHtml: state.levelUpMode ? builderRenderers.renderLevelUpNavButtons() : navButtons(),
    navButtons,
    titleCase,
    escapeHtml,
  });
...
function renderLevelUpClassChoice() {
  return builderRenderers.renderLevelUpClassChoice();
}

function renderLevelUpHpControl() {
  return builderRenderers.renderLevelUpHpControl();
}
