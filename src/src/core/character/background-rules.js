 ...
 */
function buildAbilityRule(background) {
    if (!background.abilityScores.length)
        return null;
 ...
 */
function buildSkillRule(background) {
 ...
 */
function buildToolRule(background) {
 ...
 */
function buildEquipmentRule(background) {
 ...
 */
export function backgroundChoiceRules(backgroundName, source = 'XPHB') {
    ...
    if (equipmentRule)
        rules.push(equipmentRule);
    return rules;
}
/**
 * Get all backgrounds with their rules (for UI listing)
 */
export async function getAllBackgroundRules() {
 ...
 */
export function hasMagicInitiate(backgroundName, source = 'XPHB') {
    const raw = getBackground(backgroundName, source);
    if (!raw)
        return false;
    const parsed = parseBackground(raw);
 ...
 */
export function getMagicInitiateClass(backgroundName, source = 'XPHB') {
    const raw = getBackground(backgroundName, source);
    if (!raw)
        return null;
    const parsed = parseBackground(raw);
...
//# sourceMappingURL=background-rules.js.map