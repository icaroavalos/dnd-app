 ...
 */
export function createEmptyBgChoices() {
    return {
        background: null,
        source: 'XPHB',
 ...
 */
export function resetChoicesForBackground(background) {
    return {
        abilityIncrement: null,
        abilityScores: [],
        skillChoices: [],
 ...
 */
export function getAbilityIncrements(pattern, scores) {
    const result = {
        str: 0,
        dex: 0,
        con: 0,
 ...
 */
export function calculateBackgroundAbilityBonuses(choices) {
 ...
 */
export function areBackgroundChoicesComplete(choices, requirements) {
 ...
 */
export function getMissingBackgroundChoices(choices, requirements) {
    const missing = [];
    if (!choices.background) {
 ...
 */
export function serializeBgChoices(choices) {
    const result = {};
    if (choices.background)
        result.background = choices.background;
    if (choices.source !== 'XPHB')
        result.source = choices.source;
    if (choices.abilityIncrement)
        result.abilityIncrement = choices.abilityIncrement;
    if (choices.abilityScores.length)
        result.abilityScores = choices.abilityScores;
    if (choices.skillChoices.length)
        result.skillChoices = choices.skillChoices;
    if (choices.toolChoices.length)
        result.toolChoices = choices.toolChoices;
    if (choices.equipmentChoice)
        result.equipmentChoice = choices.equipmentChoice;
    if (choices.spellcastingAbility)
        result.spellcastingAbility = choices.spellcastingAbility;
    return result;
}
/**
 * Validate ability score selection based on increment pattern
 */
export function validateAbilitySelection(selectedScores, pattern) {
    if (pattern === '2_1') {
...
//# sourceMappingURL=background-choices.js.map