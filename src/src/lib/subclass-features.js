 ...
 */
export function parseSubclassFeature(featureStr) {
    ...
    if (!name || !className || !source || !subclassName || !subclassSource)
        return null;
 ...
 */
export function getSubclassFeaturesByLevel(subclassFeatures, level) {
    if (!subclassFeatures)
        return [];
    return subclassFeatures
        .map(parseSubclassFeature)
        .filter((feat) => feat !== null && feat.level === level);
}
/**
 * Verifica se uma feature de subclasse e automatica
 */
export function isAutoSubclassFeature(featureType) {
    return featureType === 'subclass-auto';
}
//# sourceMappingURL=subclass-features.js.map