 ...
 */
export function applyShortRestRecovery(
  currentUsed: number,
  recovery: RecoveryResult['short']
): number {
  if (!recovery) return currentUsed;
 ...
 */
export function applyLongRestRecovery(_recovery: RecoveryResult['long']): number {
  return 0;
}
