...
import type { BackgroundData, RawBackground } from '../../types/background';
...
let loadPromise: Promise<BackgroundData[]> | null = null;
 ...
 */
function parseEquipmentDescription(raw: RawBackground['startingEquipment']): string[] {
  if (!raw || !Array.isArray(raw)) return [];
    ...
    if ('A' in group && Array.isArray(group.A)) {
      for (const item of group.A) {
        if (typeof item === 'string') {
          ...
          if (name) equipment.push(name.trim());
        } else if (typeof item === 'object' && item !== null && 'item' in item) {
          const itemEntry = item as { item?: string; special?: string };
          if (itemEntry.item) {
 ...
 */
function parseBackground(raw: RawBackground): BackgroundData {
  ...
  let feature: { name: string; entries: string[] } | undefined;
 ...
 */
async function loadBackgroundsFromJson(): Promise<BackgroundData[]> {
  try {
    ...
    const backgrounds: RawBackground[] = jsonData.results || jsonData.background || [];
 ...
 */
export async function loadBackgroundData(): Promise<BackgroundData[]> {
 ...
 */
export function getBackground(name: string, source: string): BackgroundData | undefined {
 ...
 */
export function getBackgroundsBySource(source: string = 'XPHB'): BackgroundData[] {
