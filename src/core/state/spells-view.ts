...
export function renderSpellsSheet(
  autoLeveledSpells: SpellItem[],
  knownSpells: any[],
  casterLevel: number,
  spellAttack: number,
  spellSaveDc: number,
  selectedSpellName: string,
  backgroundSpells: string[],
  backgroundAbilityMetrics: { attackBonus: number; saveDc: number } | null,
  ...
  ordinalLabel: (level: number) => string,
  deckLabels: Record<string, string>,
  classDecks: Record<string, string>,
  ...
  const spellDetailsByName = new Map(
    knownSpells.map((spell) => [spell.name, { ...spellFromKnownData(spell.name), ...spell }])
  );
...
function renderBackgroundSpellSource(
  spells: string[],
  metrics: { attackBonus: number; saveDc: number } | null,
  spellFromKnownData: (name: string) => any,
  ordinalLabel: (level: number) => string
): string {
  return `
...
function renderAutoGrantedSpells(
  spells: SpellItem[],
  spellFromKnownData: (name: string) => any,
  ordinalLabel: (level: number) => string
): string {
...
function renderSpellSheetGroups(
  spells: any[],
  selected: string,
  spellSlotsMaxByLevel: Record<number, number>,
  spellSlotsUsed: Record<number, { max: number; used: number }>,
  ...
  return [
    ...
    ...(slotLevels.length ? slotLevels : fallbackLevels).map((slotLevel) =>
      renderSpellSheetGroup(slotLevel, leveled.filter((spell) => spell.level <= slotLevel), selected, spellSlotsMaxByLevel, spellSlotsUsed, availableSpellSlotsAtLevel, spellLevelLabel, ordinalLabel, deckLabels, classDecks, characterClass, spellDetailsByName)
    ),
  ].join("");
}
