...
export function resourceRecoveryFromBody(body) {
    ...
    if (/regain all expended uses when you finish a short rest/i.test(text))
        ...
        recovery.short = "all";
    else if (/finish a short rest/i.test(text) && /regain all expended uses/i.test(text))
        recovery.short = "all";
    else if (/finish a short rest/i.test(text) && /regain one expended use/i.test(text))
        recovery.short = 1;
    ...
    if (/regain all expended uses when you finish a long rest/i.test(text))
        ...
        recovery.long = "all";
    else if (/regain the ability to do so when you finish a long rest/i.test(text))
        recovery.long = "all";
    else if (/you regain the ability to cast it this way when you finish a long rest/i.test(text))
        recovery.long = "all";
    else if (/you regain the ability to do so when you finish a long rest/i.test(text))
        recovery.long = "all";
    else if (/you must finish a long rest in order to cast the spell again/i.test(text))
        recovery.long = "all";
    return recovery;
}
export function resourceActionKindFromBody(body) {
    const text = String(body ?? "").toLowerCase();
    if (text.includes("bonus action"))
        return "bonus";
    if (text.includes("reaction"))
        return "reaction";
    if (text.includes("additional action"))
        return "action";
    if (text.includes("as an action"))
        return "action";
    if (text.includes("magic action"))
        return "action";
    return "";
}
export function resourceRecoveryLabel(recovery = {}) {
    if (recovery.short && recovery.long)
        return "Short or Long Rest Resource";
    if (recovery.short)
...
export function resourceMaxFromBody(body, api, character, level = 1, meta = null) {
    const text = String(body ?? "");
    ...
    if (Number.isFinite(tableValue) && tableValue > 0)
        return tableValue;
    if (/\bonce (?:you )?use this (?:trait|feature|ability)\b/i.test(text))
        return 1;
    if (/can't use (?:it|this (?:trait|feature|ability|spell(?: again)?|trait again|ability again)|that spell) again until you finish a short or long rest/i.test(text))
        return 1;
    if (/can't use (?:it|this (?:trait|feature|ability|spell(?: again)?|trait again|ability again)|that spell) again until you finish a (?:short|long) rest/i.test(text))
        return 1;
    if (/once per (?:short|long) rest/i.test(text))
        return 1;
    if (/regain the ability to do so when you finish a (?:short|long) rest/i.test(text))
        return 1;
    if (/regain the ability to cast it when you finish a long rest/i.test(text))
        return 1;
    ...
    if (proficiencyMatch)
        return Math.max(1, deriveProficiencyBonus(level));
    const fixedMatch = text.match(/\b(once|twice|thrice)\b/i);
    if (fixedMatch) {
        const fixed = { once: 1, twice: 2, thrice: 3 };
...
function classTableResourceValue(labels, level, api, charClass) {
...
function parseTableCellValue(cell) {
    ...
    if (typeof cell === "string") {
        ...
        if (!cleaned || cleaned === "—" || /^unlimited$/i.test(cleaned))
            return 0;
    ...
    if (typeof cell === "object") {
        ...
        if (typeof cell.value === "string")
            return parseTableCellValue(cell.value);
        if (typeof cell.max === "number")
            return cell.max;
    }
    return 0;
}
function abilityModifierFromLabel(label, character) {
    ...
    if (!key)
        return deriveProficiencyBonus(character.level);
    const bonuses = calculateCharacterAbilityBonuses(character);
    const scores = deriveAbilityScores(character.abilities ?? {}, bonuses);
    return deriveAbilityModifier(scores[key]);
}
//# sourceMappingURL=resource-engine.js.map