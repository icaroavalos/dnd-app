...
export function validateRuleAtom(rule) {
    ...
    if (!rule.type || !RULE_TYPES.has(rule.type))
        errors.push(`type must be one of: ${[...RULE_TYPES].join(', ')}.`);
    if (!rule.metadata || typeof rule.metadata !== 'object')
        errors.push('metadata must be an object.');
    if (rule.metadata && typeof rule.metadata.version !== 'string')
        errors.push('metadata.version must be a string.');
    if (rule.metadata?.tags && !Array.isArray(rule.metadata.tags))
        errors.push('metadata.tags must be an array when present.');
    if (rule.activation) {
        if (typeof rule.activation !== 'object')
            errors.push('activation must be an object.');
        else if (!ACTIVATION_TYPES.has(rule.activation.type))
            errors.push(`activation.type must be one of: ${[...ACTIVATION_TYPES].join(', ')}.`);
    }
    if (rule.constraints && typeof rule.constraints !== 'object')
        errors.push('constraints must be an object when present.');
    if (rule.effects && !Array.isArray(rule.effects))
        errors.push('effects must be an array when present.');
    if (rule.depends_on && !Array.isArray(rule.depends_on))
